...
{
    public class DeepSeaCrate : ModItem
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Marine Crate");
        }
        ...
        public override void RightClick(Player player)  //this make so when you right click this item, then one of these items will drop
        {
            ...
            if (Main.hardMode) // if it's hardmode this itams will drop  
            {
                ...
                if (Choose == 2)                                                  // 1 item
                {
                    player.QuickSpawnItem(mod.ItemType("MarineShard"), Main.rand.Next(3, 9));
                }
                if (Choose == 3)                                                  // 1 item
                {
                ...
                if (Choose == 6)                                                  // 1 item
                {
                    player.QuickSpawnItem(ItemID.Trident, Main.rand.Next(1, 1));
                }
                if (Choose == 7)                                                  // 1 item
                {
                    player.QuickSpawnItem(mod.ItemType("Bubble"), Main.rand.Next(3, 9));
                }
                if (Choose == 8)                                                  // 1 item
                {
            ...
            else        //else if it's prehard mode then only this items will drop
            {
                ...
                if (Choose == 2)                                                  // 1 item
                {
                    player.QuickSpawnItem(mod.ItemType("AbyssalTeslaOre"), Main.rand.Next(3, 9));
                }
                if (Choose == 3)                                                  // 1 item
                {
                    player.QuickSpawnItem(ItemID.MasterBait, Main.rand.Next(1, 5));
                ...
                player.QuickSpawnItem(mod.ItemType("MarineShard"), Main.rand.Next(1, 3));
                player.QuickSpawnItem(mod.ItemType("Skales"), Main.rand.Next(5, 24));
